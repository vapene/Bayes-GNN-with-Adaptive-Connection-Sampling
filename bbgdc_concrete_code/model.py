...
class BBGDC(nn.Module):
    def __init__(self, num_pars, alpha=0.8, kl_scale=1.0):
        ...
        self.num_pars = num_pars # 1
        self.alpha = alpha # c/L in paper
    ...
    def get_params(self):
        a = F.softplus(self.a_uc.clamp(min=-10.)) # relu 비슷한 함수
        b = F.softplus(self.b_uc.clamp(min=-10., max=50.))
        return a, b # 1~1.5, 0.5
    
    def sample_pi(self):
        a, b = self.get_params() # 0.6931, 0.6931
        u = torch.rand(self.num_pars).clamp(1e-6, 1-1e-6) # 0.001~0.999
        if torch.cuda.is_available():
            u = u.cuda()
        return (1 - u.pow_(1./b)).pow_(1./a)   # p -2.1972 in CONCRETE
                                                # random 0~1  here
...
class BBGDCGCN(nn.Module):
    def __init__(self, nfeat_list, dropout, nblock, nlay, num_edges):
        ...
        self.nlay = nlay #4
        self.nblock = nblock #2
        self.num_edges = num_edges # 2708 ^2
        self.num_nodes = int(np.sqrt(num_edges)) # 2708
        self.drpcon_list = []
